...
!print *,'nresx',nresx, 'Q_in', Q_res_in(nresx),'fttom', ftsec_to_msec, 'dtcomp',dt_comp
        ...
        if ( density_in(nresx) .le. density_hypo(nresx) ) then
                flow_in_hyp_x = Q1 * 0 ! 0.2 
                flow_in_epi_x = Q1 * 1 ! 0.8 majority of flow goes to epil.
        else
                flow_in_hyp_x = Q1 * 1  ! 0.8
                flow_in_epi_x = Q1 * 0 ! 0.2 majority flow goes to hypo
        end if
     ...
     !      flow_epi_hyp_x = 0
        ...
        if(nresx .eq. 8) write(77,*), volume_e_x(nresx), volume_h_x(nresx) &
